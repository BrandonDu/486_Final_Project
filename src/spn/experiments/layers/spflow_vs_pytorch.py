...
def run_spflow(spflow_spn, n_feats, batch_size, repetitions):
    print("Running SPN with: nfeat=%s, batch=%s" % (n_feats, batch_size))
...
def run_spflow_layers(layers, n_feats, batch_size, repetitions):
    print("Running SPN layers with: nfeat=%s, batch=%s" % (n_feats, batch_size))
    x = np.random.rand(batch_size, n_feats).astype(np.float32)
...
def run_spflow_layers_torch(spn, n_feats, batch_size, repetitions):
    print("Running SPN layers torch with: nfeat=%s, batch=%s" % (n_feats, batch_size))
    device = "cuda"
    x = torch.rand(batch_size, n_feats, 1).to(device)
...
def visualize():
    ...
    #axes[0, 0].plot(spflow_batch[:, 0], spflow_batch[:, 1], label="SPN")
    #axes[0, 0].plot(tf_batch[:, 0], tf_batch[:, 1], label="SPN-TF")
    #axes[0, 0].plot(pytorch_batch[:, 0], pytorch_batch[:, 1], label="Layerwise PyTorch")
    #axes[0, 0].plot(spflow_layers_batch[:, 0], spflow_layers_batch[:, 1], label="SPN Layers")

    #axes[0, 1].plot(spflow_nfeat[:, 0], spflow_nfeat[:, 1], label="SPN")
    #axes[0, 1].plot(tf_nfeat[:, 0], tf_nfeat[:, 1], label="SPN-TF")
    #axes[0, 1].plot(pytorch_nfeat[:, 0], pytorch_nfeat[:, 1], label="Layerwise PyTorch")
    #axes[0, 1].plot(spflow_layers_nfeat[:, 0], spflow_layers_nfeat[:, 1], label="SPN Layers")
    ...
    #     label="SPN/Layerwise PyTorch",
    # )
    # axes[1, 0].plot(
    #     tf_batch[:, 0],
    #     tf_batch[:, 1] / pytorch_batch[:, 1],
    #     label="SPN-TF/Layerwise PyTorch",
    # )
    # axes[1, 1].plot(
    #     spflow_nfeat[:, 0],
    ...
    # Titles
    title = "vs".join(names) + " SPN Forward" #"SPN vs SPN-TF vs PyTorch: SPN Forward Pass"
    fig.suptitle(title)
